      ...
      <div className="flex justify-between items-start border-b border-zinc-800 pb-5 mb-5">
        <div>
          ...
          <p className="text-zinc-500 text-xs font-semibold uppercase tracking-wider mt-1">
            {t('history_title')} • {data.sellerData.firstname} {data.sellerData.lastname}
          </p>
        </div>
        <button 
          onClick={onCancel} 
      ...
      <div className="flex-1 grid grid-cols-12 gap-6 overflow-hidden">
        ...
        <div className="col-span-7 flex flex-col h-full overflow-hidden space-y-4">
          ...
          <div className="flex bg-zinc-900/60 p-1.5 rounded-2xl border border-zinc-800/80">
            ...
            >
              <Tag size={14} /> {t('history_tab_active')} ({data.active?.length || 0})
            </button>
            <button
            ...
            >
              <Receipt size={14} /> {t('history_tab_sold')} ({data.sold?.length || 0})
            </button>
          </div>
            ...
            {activeTab === 'active' ? (
              data.active && data.active.length > 0 ? (
                ...
                <div className="h-full flex flex-col items-center justify-center text-zinc-500 py-16">
                  ...
                  <p className="text-xs font-semibold uppercase tracking-wider">{t('history_empty_active')}</p>
                </div>
              )
            ) : (
              data.sold && data.sold.length > 0 ? (
                ...
                <div className="h-full flex flex-col items-center justify-center text-zinc-500 py-16">
                  ...
                  <p className="text-xs font-semibold uppercase tracking-wider">{t('history_empty_sold')}</p>
                </div>
              )
            )}
          </div>
          ...
          {activeTab === 'active' && selectedActive ? (
            <div className="flex flex-col h-full">
              ...
              <div className="flex-1 space-y-5 overflow-y-auto pr-1 custom-scrollbar">
                ...
                <div className="space-y-3">
                  <div>
                    <span className="text-[10px] font-black uppercase tracking-widest text-zinc-500 block">{t('contract_vehicle')}</span>
                    <span className="text-white font-black text-base uppercase leading-none">{selectedActive.model}</span>
                  </div>
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      <span className="text-[10px] font-black uppercase tracking-widest text-zinc-500 block">{t('history_plate')}</span>
                      <span className="text-zinc-300 font-bold uppercase text-xs">{selectedActive.plate}</span>
                    </div>
                    <div>
                      <span className="text-[10px] font-black uppercase tracking-widest text-zinc-500 block">{t('history_price_charged')}</span>
                      <span className="text-concessionaire font-black text-sm">{formatPrice(selectedActive.price)}</span>
                    </div>
                  </div>
                  {selectedActive.description && (
                    <div>
                      <span className="text-[10px] font-black uppercase tracking-widest text-zinc-500 block">{t('history_seller_desc')}</span>
            ...
            <div className="flex flex-col h-full">
              ...
              <div className="flex-1 space-y-4 overflow-y-auto pr-1 custom-scrollbar">
                ...
                <div className="space-y-3 bg-zinc-950/40 border border-zinc-800/60 p-4 rounded-2xl">
                  <div className="flex items-center gap-2 border-b border-zinc-800/50 pb-2">
                    ...
                    <div className="flex-1 min-w-0">
                      <span className="text-[8px] font-black uppercase tracking-wider text-zinc-500 block">{t('history_buyer')}</span>
                  ...
                  <div className="flex items-center gap-2 border-b border-zinc-800/50 pb-2">
                    ...
                    <div>
                      <span className="text-[8px] font-black uppercase tracking-wider text-zinc-500 block">{t('history_date')}</span>
                  ...
                  <div className="flex items-center gap-2 pb-1">
                    ...
                    <div>
                      <span className="text-[8px] font-black uppercase tracking-wider text-zinc-500 block">{t('history_credit')}</span>
              ...
              <div className="mt-auto pt-4 border-t border-zinc-800/80 flex flex-col gap-2">
                ...
                >
                  <FileText size={16} /> {t('history_view_contract')}
                </Button>
                <Button 
        ...
        <div className="absolute inset-0 z-[110] flex items-center justify-center p-8 animate-in fade-in duration-300">
          ...
          <div className="relative bg-zinc-950 border border-zinc-800 p-8 rounded-3xl shadow-2xl w-full max-w-md animate-in zoom-in-95 duration-300">
             <div className="flex items-center justify-between mb-4">
                <h3 className="text-xl font-black uppercase tracking-tighter text-white">{t('history_delete_title')}</h3>
             ...
             <p className="text-zinc-500 text-sm mb-6 leading-relaxed" dangerouslySetInnerHTML={{
                __html: t('history_delete_msg').replace('%s', `<span class="text-white font-bold">${selectedSold.model.toUpperCase()}</span>`).replace('%s', selectedSold.plate)
             }} />
