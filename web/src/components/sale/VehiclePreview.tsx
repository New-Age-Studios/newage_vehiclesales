    ...
    >
      ...
      {!hasCustomPhoto && onTakePhoto && (
        <div className="absolute inset-0 flex flex-col items-center justify-center bg-black/60 backdrop-blur-sm group-hover:bg-black/40 transition-all duration-300">
          ...
          <span className="text-[10px] font-black text-white uppercase tracking-widest">{t('sale_take_photo')}</span>
          <span className="text-[8px] font-bold text-red-400 uppercase mt-0.5 tracking-wider">{t('sale_photo_mandatory')}</span>
        </div>
      )}
      ...
      {hasCustomPhoto && (
        <div className="absolute bottom-4 left-4 flex items-center space-x-2">
          ...
          <span className="text-[10px] font-black text-white uppercase tracking-widest">
            {onTakePhoto ? t('sale_photo_captured_retry') : t('sale_photo_captured')}
          </span>
        </div>
      )}
    </button>
